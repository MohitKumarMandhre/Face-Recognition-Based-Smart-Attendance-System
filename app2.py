...
def Attendance():
    ...
    list_of_files = glob.glob('D:/New_project/p4c/Face-Recognition-Based-Smart-Attendence-System/Attendance/*')
...
def SentMail():
    ...
    list_of_files = glob.glob('D:/New_project/p4c/Face-Recognition-Based-Smart-Attendence-System/Attendence/*')
    latest_file = max(list_of_files, key=os.path.getctime)
...
def send_mail():
    ...
    server.login('mohitmandhre@gmail.com', 'hhzhdmvhmmmspasi')

    subject = "( Beta Pahuch Gya ! )"
    body = f'YOUR WARD HAS REPORTED TO COLLEGE FOR DATE - { date } AT TIME - { Hour } HOURS { Minute } MINUTES { Second } SECONDS'
    msg = f"Subject: {subject}\n\n{body}" 

    server.sendmail(
        'mohitmandhre@gmail.com',
        'harshamandhre@gmail.com',
        msg
    )
    ...
    def popupmsg():
        ...
        popup.geometry("+{}+{}".format(positionRight, positionDown))
        popup.wm_title("Notification")
        label = ttk.Label(popup, text="Successfully sent mail to Ghar Wale")
        label.pack(side="top", fill="x", pady=50)
        B1 = ttk.Button(popup, text="Close", command=popup.destroy)
        B1.pack()
        popup.mainloop()
...
quitWindow = tk.Button(window, text="Sent Attendance(Mail)", command=send_mail, fg="white", bg="Blue",
                       width=20, height=2, activebackground="white", font=('times', 15, ' bold '), relief="raised")
quitWindow.place(x=930, y=600)

copyWrite = tk.Label(window, text="Created By SSITM CSE Batch ( 2016-2020 ) \n ( MOHIT KUMAR MANDHRE & SIDDHANT BOTHRA )",
